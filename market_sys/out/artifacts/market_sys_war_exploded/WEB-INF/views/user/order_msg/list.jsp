...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=0306"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/order_msg/queryList",
		data : {
			"page":page,
			"orderNo":orderNo,
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				html+='<td>';
				if(rows[i].orderMsg.orderStatus=='3'){
					html+='<a href="javascript:void(0)" onclick="qrsh(this)" data-id="'+rows[i].orderMsg.id+'" act-type="1" class="btn btn-danger btnStyle">确认收货</a>';
				}
				html+='</td>';
				html+='</tr>';
			}
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/user/order_msg/queryCount",
		async: false,
		data : {
			"orderNo":orderNo,
			"orderStatus":orderStatus
...
function qrsh(e){
	var r=confirm("确认确认收货该订单？");
	if(r==true){
		var id = $(e).attr("data-id");
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/user/order_msg/qrsh",
			data : {
				"id":id
			},
			success : function(result) {
