...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=4685"></script>
	...
	$.ajax({
		...
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				var birthdayVal=setNullToEmpty(rows[i].userMsg.birthday);
				html+='<td>'+ birthdayVal+'</td>';
				var sexVal=setNullToEmpty(rows[i].sexStr);
				html+='<td>'+sexVal+'</td>';
				var addressVal=setNullToEmpty(rows[i].userMsg.address);
				html+='<td>'+ addressVal+'</td>';
				var userScoreVal=setNullToEmpty(rows[i].userMsg.userScore);
				html+='<td>'+ userScoreVal+'</td>';
