...
<meta name="description" content="商品类型">
			...
			<div class="ibox float-e-margins">
				...
				<div class="ibox-content">
					<div class="row">
						<div class="col-lg-2">
							<label>类型名</label>
							<input type="text" class="form-control" value="" placeholder="请输入类型名" id="typeName" >
						</div>
						<div class="col-lg-1">
					...
					<div class="row"  style="margin-top:5px;padding-left: 15px;">
						<a href="${pageContext.request.contextPath}/admin/pro_type/add" class="btn btn-primary">新增</a>
					</div>
					...
					<div class="table-responsive">
						<table class="table  table-striped table-bordered  table-hover">
							...
							<tbody  id="pro_type_body">
							</tbody>
						</table>
						<div id="pro_type_bar" class="pagination" style="margin-top:20px;"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript" src="${pageContext.request.contextPath}/static/common/utils/listutils.js?v=5805"></script>
...
function ajaxList(page) {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/pro_type/queryList",
		data : {
			"page":page,
			"typeName":typeName
		},
		success : function(result) {
			...
			for (var i = 0; i < rows.length; i++) {
				html += '<tr>' ;
				var typeNameVal=setNullToEmpty(rows[i].proType.typeName);
				html+='<td>'+ typeNameVal+'</td>';
				html+='<td>';
				html+='<a href="${pageContext.request.contextPath}/admin/pro_type/update?id='+rows[i].proType.id+'" class="btn btn-success btnStyle">修改</a>';
				html+='<a href="javascript:void(0)" onclick="del(this)" data-id="'+rows[i].proType.id+'" class="btn btn-danger btnStyle">删除</a>';
				html+='</td>';
				html+='</tr>';
			}
			if (rows.length == 0) {
...
function ajaxCount() {
	...
	$.ajax({
		type : 'get',
		url : "${pageContext.request.contextPath}/admin/pro_type/queryCount",
		async: false,
		data : {
			"typeName":typeName
		},
...
function del(e){
	if(window.confirm('你确定要删除该商品类型吗')){
		...
		$.ajax({
			type : 'get',
			url : "${pageContext.request.contextPath}/admin/pro_type/del",
			data : {
				"id":id
			},
			success : function(result) {
